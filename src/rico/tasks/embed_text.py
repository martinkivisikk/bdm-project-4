...
from rico.utils import compute_fingerprint, get_postgres_conn
...
def embed_text_task(**context) -> None:
    ...
    with get_postgres_conn() as conn:
        ...
        with conn.cursor() as cur:
            for sid, text, vec in zip(screen_ids, parsed_texts, vecs_np):
                fp = compute_fingerprint(text.encode("utf-8"))
                cur.execute(
                    _UPSERT_SQL,
                    (sid, _MODEL_NAME, _MODEL_VERSION, "text", vec, run_id, fp),
                )
                log.info(
                    "[%s] embed_text screen=%d  dim=%d  norm=%.4f",
